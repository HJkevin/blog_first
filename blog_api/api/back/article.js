...
// 插入文章
router.post('/inser', (req, res, next)=>{
  var sql = `inser into ${req.body.enname_one}(id, oneId, twoId, article_name, editer, content, time, visitors, daodu, imgsrc, art_show) values('${Unique()}', '${req.body.oneId}', '${req.body.twoId}', '${req.body.article_name}', '${req.body.editer}', '${req.body.content}', '${req.body.time}', 0, '${req.body.daodu}', '${req.body.imgsrc}', '${req.body.recommend}', '${req.body.art_show}')`

  console.log(sql)
  var updateArticalNum = `update two_class set article_num=article_num + 1 where id '${req.body.twoId}'`
    const asyncInserArticle = async function(){
      await sqlHandle(sql)
      await sqlHandle(updateArticalNum)
      return 
    }

    asyncInserArticle().then((data)=>{
      res.send({
          code:'3011',
          msg:'插入文章成功'
        })
...
router.post('/updateArticle', (req, res, next)=>{
  var sql = `update ${req.body.enname_one} set article_name = '${req.body.article_name}', editer = ${req.body.editer}, content='${req.body.content}', time='${req.body.time}', visitors = ${req.body.visitors}, daodu = '${req.body.daodu}', imgsrc = '${req.body.imgsrc}', art_show='${req.body.art_show}' where id = '${req.body.id}'`
