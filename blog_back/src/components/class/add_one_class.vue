<template>
  <div class="classmain">
    <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="130px" class="demo-ruleForm">
      <h2>一级分类列表</h2>
      <el-form-item label="一级中文标识" prop="cnname_one" :rules="[{required: true, message: '不能为空'}]">
      ...
      <h2>二级分类列表</h2>
      <el-form-item label="二级中文标识" prop="cnname_two" :rules="[{required: true, message: '不能为空'}]">
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        enname_one: [{ validator: validateEnname_one, trigger: "blur" }],
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.axios
            .post("/api/back/class/insertOneClass", this.ruleForm)
            .then(data => {
              console.log(data);
